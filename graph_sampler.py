...
class GraphSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, G_list, features='default', normalize=True, assign_feat='default', max_num_nodes=0):
        ...
        if features == 'default':
            ...
            self.feat_dim = len(util.node_dict(G_list[0])[0]['label'])
        ...
        for G in G_list:
            ...
            # Default: use original node features (if exists)
            if features == 'default':
                f = np.zeros((self.max_num_nodes, self.feat_dim), dtype=float)
                for i,u in enumerate(G.nodes()):
                    f[i,:] = util.node_dict(G)[u]['feat']
                self.feature_all.append(f)
            # Label: use node labels
            elif features == 'label':
                f = np.zeros((self.max_num_nodes, self.feat_dim), dtype=float)
                for i,u in enumerate(G.nodes()):
                    f[i,:] = util.node_dict(G)[u]['label']
                self.feature_all.append(f)
            # Id: use node id
            elif features == 'id':
                self.feature_all.append(np.identity(self.max_num_nodes))
            # Deg-num: use degree of nodes
